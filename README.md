...
# ABC250B Enlarged Checker Board　　済
...
S=input() SS=S* 10とすればSの長さによらず条件を満たす文字列を先頭にした文字列が出力できる
...
➀ int(X,base=N) で N進数の文字列Xを10進数の整数に変換する。baseを指定しない場合Xは整数でもよいが、baseを明示している場合は文字列型である必要がある。
