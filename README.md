...
N=2**50-1 において log(N)/log(2)は49.9999999....のように決して50にはならないが限りなく近い数として表されるはずであるが50が出力されてしまう。理由はわからないがほぼ収束している範囲内の入力における小さな差が結果に反映されていない。(np.exp(-1000)とnp.exp(-1001)も0.0が表示されてしまう。)



➁ bit演算による桁数の判定

10**N≦X<10**(N+1)を満たすXの桁数はN+1である。同様に,2**N<=X(2)<"**(N+1)を満たすX(2)の桁数は(N+1)である。
